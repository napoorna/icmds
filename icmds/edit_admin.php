...
if (isset($_SESSION['icmds_login'])) {
  ...
  if ((isset($_GET['adminid']) && isset($_GET['email'])) || isset($_POST['updateadmin'])) {

    if (isset($_GET['adminid']) && isset($_GET['email'])) {
      $adminid = $mysqli->real_escape_string($_GET['adminid']);
      $email = $mysqli->real_escape_string($_GET['email']);
      $sql = $mysqli->query("SELECT * FROM admin WHERE email='$email' AND id='$adminid'");
      if ($sql->num_rows == 0) {
  ...
  if (isset($_POST['updateadmin'])) {
    ...
    if ($_POST['password']=="") {
      $update = "UPDATE admin SET email='$email', name='$name' WHERE id='$adminid'";
    } else {
      $password = md5($mysqli->real_escape_string($_POST['password']));
      $update = "UPDATE admin SET email='$email', name='$name', password='$password' WHERE id='$adminid'";
    }

      if ($mysqli->query($update)) {
        echo '<script language="javascript">';
        echo 'alert("Account Details Updated Successfully");';
        echo 'window.location.href = "admins"';
        echo '</script>';
      } else {
        echo '<script language="javascript">';
    ...
    <section class="content">
        <div class="container-fluid">
            <div class="row">
              <div class="card">
                <div class="body">
                  <center><h3>Modify Admin</h3></center><br>
                  <form id="form_validation" method="POST" action="edit_admin">
                      <div class="col-sm-12">
                        <div class="col-sm-6">
                      ...
                      <center><button class="btn btn-primary waves-effect" type="submit" name="updateadmin">Update Details</button></center>
                  </form>
                </div>
              </div>
            </div>
