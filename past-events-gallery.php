<?php
  if ($_GET['event_id']) {
    ...
    $sql = $mysqli->query("SELECT id FROM eventdocs WHERE event_id='$eventid'");
    if ($sql->num_rows == 0) {
      header('location: past-events');
    } else {
